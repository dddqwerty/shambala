  ...
  return (
    <div className="ml-20 z-50 h-[256px] w-[218px]">
      <div className="flex flex-col gap-3 overflow-hidden relative">
        <div
          className="relative bg-white w-[256px] h-[218px] rounded-md transform overflow-hidden transition hover:bg-gray-800"
          onMouseEnter={() => {
            if (!anim) {
              setAnim(true)
              animControls[0]
        ...
        >
          ...
          <motion.div
            ...
            className="absolute -right-[40%] -bottom-[20%] bg-blue-600 h-[80%] w-[100%] z-50 -rotate-30"
            onAnimationComplete={() => setAnim(false)}
          />
          <motion.div
            animate={animControls[1]}
            initial={{ x: -100, rotate: -45 }}
            className="absolute -left-[50%] bottom-[50%] bg-blue-600 h-[100%] w-[100%] z-50 -rotate-45"
            onAnimationComplete={() => setAnim(false)}
          />
          <motion.div
            animate={animControls[2]}
            initial={{ x: -100, rotate: -45, skew: '45deg' }}
            className="absolute -left-[5%] bottom-[5%] bg-white h-[20%] w-[80%] z-50 -rotate-45 rounded-md"
            onAnimationComplete={() => setAnim(false)}
          />
          <motion.div
            animate={animControls[3]}
            initial={{ x: -100, rotate: -45, skew: '45deg' }}
            className="absolute -right-[40%] top-0 bg-yellow-300 h-[20%] w-[45%] z-50 rotate-45 rounded-md"
            onAnimationComplete={() => setAnim(false)}
          />
          {/* <motion.div
            animate={animControls}
            className="absolute right-[596px] bg-blue-400 h-[100px] w-24 z-50 rotate-60"
            onAnimationComplete={() => setAnim(false)}
          /> */}
        </div>
